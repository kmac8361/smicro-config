...
internal_ip="172.27.${llcserver_number}.1"
...
[ -f /var/lib/maas/.ssh/known_hosts ] || sudo -u maas ssh-keyscan 172.27.${llcserver_number}.1 |sudo -u maas tee -a /var/lib/maas/.ssh/known_hosts > /dev/null

# Always add host keys .. not an issue if files grows
sudo -u maas ssh-keyscan 172.27.${llcserver_number}.1 |sudo -u maas  tee -a /var/lib/maas/.ssh/known_hosts > /dev/null
...
sudo -u maas virsh -c qemu+ssh://ubuntu@172.27.${llcserver_number}.1/system list > /dev/null || sed -i '/ maas@/d' /home/ubuntu/.ssh/authorized_keys
